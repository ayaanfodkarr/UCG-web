    ...
    entries.forEach(entry => {
        if (entry.isIntersecting) {
            ...
            if (entry.target.classList.contains('features-grid') || 
                ...
                const items = entry.target.querySelectorAll('.feature-card, .service-card, .industry-item');
...
function updateHeroParallax() {
    ...
    if (hero) {
        const rate = heroCurrentScroll * -0.3;
...
document.addEventListener('DOMContentLoaded', () => {
    const cards = document.querySelectorAll('.feature-card, .service-card, .industry-item, .contact-item');
    
    cards.forEach(card => {
        card.addEventListener('mouseenter', (e) => {
            // Simple, clean hover effect - let CSS handle the styling
        });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Remove scroll indicator on scroll
    window.addEventListener('scroll', () => {
        if (window.scrollY > 100) {
